...
q: Qlearning = Qlearning(maze = Maze.from_solvedmaze(maze), hyperparams = {}, strategy=Strategy.SOFTMAX)
q.run()


for i in range(q.hyperparams["episodes"]):
    print(f"epoch {i} steps {len(q.env.history.steps[i])} reward {np.sum(q.env.history.rewards[i])}")
